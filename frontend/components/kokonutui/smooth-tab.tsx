...
export default function SmoothTab({
  ...
  const gridColsClass = items.length === 3 ? "grid-cols-3" : items.length === 4 ? "grid-cols-4" : `grid-cols-${items.length}`;

  return (
    <div className="flex h-full flex-col">
      {/* Card Content Area - only show if showContent is true */}
      {showContent && (
      ...
      <div
        ...
        className={cn(
          ...
          showContent ? "mt-auto mx-auto w-[400px] border" : "",
          className
        )}
        ref={containerRef}
        role="tablist"
      >
        ...
        <motion.div
          ...
          className={cn(
            "absolute z-[1] rounded-full shadow-md",
          ...
          {items.map((item) => {
            ...
            return (
              <motion.button
                ...
                className={cn(
                  "relative flex items-center justify-center gap-2 rounded-full px-6 py-3",
