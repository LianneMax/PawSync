  ...
  const handleGoogleSignIn = useGoogleLogin({
    onSuccess: async (tokenResponse) => {
      ...
      try {
        ...
        if (response.status === 'NEEDS_USER_TYPE') {
          // No account found — send them to signup with Google data pre-filled
          sessionStorage.setItem('googlePendingUser', JSON.stringify(response.data))
          router.push('/signup?via=google')
          return
        }
        ...
        <div className="w-full md:w-3/5 p-12 flex flex-col justify-center">
          <div className="max-w-md mx-auto w-full">
            ...
            <form onSubmit={handleSubmit} noValidate>
              {/* Error Message */}
              {error && (
                <div className="mb-4 p-4 bg-[#F4D3D2] border border-[#900B09]/20 text-[#900B09] rounded-xl">
                  {error}
                </div>
              )}
              ...
              <div className="mb-4">
                <div className="relative">
                  ...
                  <input
                    ...
                    className={`w-full pl-12 pr-4 py-4 bg-gray-100 rounded-xl border-2 ${fieldErrors.email ? 'border-[#900B09]/20' : 'border-transparent'} focus:outline-none focus:ring-2 focus:ring-[#7FA5A3] transition-all`}
                  />
                </div>
          ...
          >
              ...
              <div>
                ...
                <div className="flex gap-4 items-start">
                  <div className="flex-1">
                    {modalError && (
                      <p className="text-[#900B09] text-sm mb-2">{modalError}</p>
                    )}
            ...
            {activeModal === 'otp' && (
              <div>
                ...
                {modalError && (
                  <p className="text-[#900B09] text-sm mb-3">{modalError}</p>
                )}
