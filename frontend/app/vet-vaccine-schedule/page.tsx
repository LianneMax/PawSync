...
export default function VetVaccineSchedulePage() {
  ...
  useEffect(() => {
    async function fetchSchedule() {
      ...
      setLoading(true)
      try {
        const data = await getVetUpcomingSchedule(user.id, token)
  ...
  const groupedByPet = filteredSchedule.reduce(
    ...
    {} as Record<string, { pet: { _id: string; name: string; species: string; breed: string; ownerId: string; photo?: string }; vaccines: VetUpcomingSchedule[] }>
  );
